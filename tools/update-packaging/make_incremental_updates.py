...
def process_explicit_remove_files(dir_path, patch_info):
    ...
    list_file_path = os.path.join(dir_path, "removed-files.tmp")
    if not os.path.exists(list_file_path):
        list_file_path = os.path.join(dir_path, "Contents/Resources/removed-files.tmp")
...
def create_partial_patch(from_dir_path, to_dir_path, patch_filename, shas, patch_info, forced_updates, add_if_not_list):
    ...
    # Windows and linux have this file at the root of the dir
    removed_files_path = os.path.join(to_dir_path, "removed-files")
    if not os.path.exists(removed_files_path):
        # On Mac removed-files contains relative paths from Contents/MacOS/
        removed_files_path = os.path.join(to_dir_path, "Contents/Resources/removed-files")

    if (os.path.exists(removed_files_path)):
        shutil.copy2(removed_files_path, removed_files_path+".tmp")
    ...
    if (os.path.exists(removed_files_path+".tmp")):
        os.remove(removed_files_path+".tmp")
...
def create_partial_patches(patches):
    ...
    try:
        ...
        for patch in patches:
            ...
            partial_filename = create_partial_patch(work_dir_from, work_dir_to, patch_filename, shas, PatchInfo(work_dir, ['update.manifest','updatev2.manifest','updatev3.manifest','removed-files.tmp'],['/readme.txt']),forced_updates,['channel-prefs.js','update-settings.ini'])
